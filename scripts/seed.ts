 ...
 *
 * Seeds the local D1 database via wrangler d1 execute.
...
const checkResult = execSync(
  `wrangler d1 execute kilat --local --command "SELECT id FROM users WHERE email = '${email.replace(/'/g, "''")}'" --json`,
  { encoding: "utf8" },
);
...
execSync(
  `wrangler d1 execute kilat --local --command "INSERT INTO users (name, email, password_hash, role) VALUES ('${safeName}', '${safeEmail}', '${safeHash}', '${role}')"`,
  { stdio: "inherit" },
);
