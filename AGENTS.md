...
- **Inertia v3 + Vue 3** — in-process SSR via `@vue/server-renderer`; page
  registry in `src/client/pages.ts` with explicit imports. `renderToString`
  from `@vue/server-renderer` runs inside a pre-built SSR bundle
...
- **esbuild** — client asset bundler (`scripts/build.ts`). Two esbuild
  passes: client (`app.ts` with `vuePlugin()`) and SSR (`ssr.ts` with
  `vuePlugin({ ssr: true })` → `dist/ssr.js`). A Tailwind v4 CLI pre-step
  runs before esbuild. Emits content-hashed JS + CSS to `dist/`, served via
  Workers Static Assets binding (`env.ASSETS`).
- **Tailwind CSS v4 + scoped `<style>`** — `styles.css` holds only global
  base tokens (CSS variables, reset, shared UI primitives); `tailwind.css`
  is the Tailwind entry point with `@import "tailwindcss"`,
  `@custom-variant dark`, and `@theme inline` bridging CSS vars to Tailwind
  tokens. Scoped `<style>` blocks in `.vue` SFCs handle complex component
  styles alongside Tailwind utilities. The Tailwind CLI runs as a pre-build
  step, producing `.tailwind.css` which is imported by `app.ts` (see "CSS"
  below).
...
scripts/
├── build.ts                # esbuild: Tailwind CLI pre-step + two passes (client + SSR) → dist/assets/app-[hash].js + CSS + manifest.json, dist/ssr.js
...
6. **TypeScript**: `strict` + `noUncheckedIndexedAccess` +
   ...
   No ORM, no loose `any`; queries are parameterized. Typecheck with
   `vue-tsc` (not `tsc`) so `.vue` SFCs are type-checked.

7. **CSS is co-located, not centralised.** Tailwind CSS v4 is the primary
   styling approach. `tailwind.css` is the Tailwind entry point:
   `@import "tailwindcss"`, `@custom-variant dark` for dark mode,
   `@theme inline` bridging CSS vars to Tailwind tokens so dark mode
   auto-switches at runtime. `styles.css` holds only global base: design
   tokens, reset, `:focus-visible`, and shared UI primitives. Scoped
   `<style>` blocks in `.vue` SFCs are used for complex component styles
...
- Suite must stay green: run `bun run typecheck` and
  `bun run test` before finishing. `vue-tsc` covers `src/` and `scripts/`
  including `.vue` SFCs.
