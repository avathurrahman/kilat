...
**Free to start.** 100K requests/day, 500 MB D1 database, 20K static assets —
all on Cloudflare's free tier. No credit card, no server to rent, no database
to provision. See the full [free tier limits](/limits/).

Outgrew the free tier? [Dulak](https://dulak.pages.dev) is Kilat's sister
...
[migration guide](/blog/migrating-to-dulak/) — there's even a copy-paste
...
**Bun is the package manager and script runner — the app does NOT run on Bun.**
It runs on the Workers runtime via Wrangler/Miniflare.
...
No Bun? `npx create-kilat@latest my-app` works too (falls back to npm install),
but you'll need [Bun](https://bun.sh) for the test suite and build scripts.
