...
- **Bun >= 1.3** — package manager and script runner ([install](https://bun.sh)).
  The app runs on Cloudflare Workers, not Bun — Bun installs deps and runs
  scripts (build, test, typecheck). `npm install` works as a fallback for
  installing deps, but **scripts require Bun** (`bun run test` uses
  `bun:test`, which npm cannot run).
...
Don't have Bun yet? `npx create-kilat@latest my-app` works too — it falls
back to `npm install` automatically. But you'll need [Bun](https://bun.sh)
to run the test suite and build scripts.
...
bun create kilat my-app --remote --template default   # also create KV + R2
```
