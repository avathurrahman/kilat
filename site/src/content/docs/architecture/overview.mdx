...
```
`initConfig`, `initDb`, and `initSessionCache` run **per-request** — they
mutate module-level singletons via cheap pointer assignments. Workers isolates
are stateless; do not cache state across requests. `initSessionCache` is a
no-op when `SESSION_KV` is absent (tests, local dev without the binding). The
app is built once at module load (the `createApp(assets)` call); only config,
DB, and session KV are re-bound each request.
